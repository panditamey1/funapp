...
def save_predefined_list(numbers_list, file_name='custom_predefined_numbers.csv'):
...
# Select predefined list or enter a custom one
selected_list_name = st.selectbox('Select a predefined list or enter a custom list:', list(list_options.keys()))

if selected_list_name == 'Custom Define':
    # Allow user to define a custom list
    custom_list_input = st.text_area('Enter your custom list of numbers, separated by commas:')
    if st.button('Update Custom List'):
        try:
            custom_list = [int(x.strip()) for x in custom_list_input.split(',')]
            list_options['Custom Define'] = custom_list
            save_predefined_list(custom_list)
            st.success('Custom predefined list updated successfully.')
        except ValueError:
            st.error('Please enter valid integers separated by commas.')
else:
    if st.button('Save Selected List'):
        save_predefined_list(list_options[selected_list_name], file_name=f"{selected_list_name.lower().replace(' ', '_')}_numbers.csv")
        st.success(f"'{selected_list_name}' list saved successfully.")

# Display the selected or updated list
st.write('Current predefined list:', list_options[selected_list_name])

# Example of using the selected list for some functionality
...
if selected_file:
    ...
    if st.button('Show Analysis') and numbers_list:
...
# File management and other Streamlit functionality can be added as needed.
